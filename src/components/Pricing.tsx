  ...
  return (
    <section id="pricing" className="py-20 md:py-32 bg-surface flex justify-center">
      <div className="w-full max-w-7xl px-4 md:px-12">
        <div className="mb-20 md:mb-20 max-w-2xl px-2 md:px-0">
          <span className="text-[12px] uppercase tracking-[0.5em] text-muted font-black mb-4 block">Tailored Global Solutions</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 md:gap-8">
          {PLANS.map((plan, i) => (
            <motion.div
              ...
              className={`flex flex-col p-8 md:p-10 justify-between min-h-[400px] md:min-h-[500px] border rounded-2xl md:rounded-3xl transition-all duration-500 bg-background ${
                plan.highlight 
            ...
            >
              <div>
                <h3 className={`text-[12px] uppercase tracking-widest mb-4 md:mb-6 font-black ${plan.highlight ? 'text-foreground' : 'text-muted'}`}>
                  {plan.name}
                </h3>
                <div className="text-4xl md:text-5xl font-black tracking-tighter mb-2 md:mb-4 text-foreground">
                  {plan.price}
                </div>
                <p className="text-[12px] uppercase tracking-widest text-muted font-bold leading-relaxed mb-6 md:mb-10">
                  {plan.tagline}
                </p>
                
                <div className="w-8 h-[1px] bg-foreground/20 mb-6 md:mb-10" />
                
                <ul className="text-[12px] uppercase tracking-widest space-y-4 md:space-y-4 text-muted">
                  {plan.features.map((feature) => (
                    <li key={feature} className="flex items-start gap-2 md:gap-3">
                      <Check size={12} className="mt-0.5 text-foreground md:w-3 md:h-3" />
                      {feature}
                    </li>
                  ))}
                </ul>
              ...
              <button className={`w-full py-4 md:py-5 mt-10 md:mt-12 text-[12px] font-black uppercase tracking-[0.3em] rounded-full transition-all ${
                plan.highlight 
